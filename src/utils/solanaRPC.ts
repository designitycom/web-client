import {
  ...
  export default class SolanaRpc {
    ...
    airDrop = async (): Promise<number> => {
      try { 
        console.log("air Drop start");
        const solanaWallet = new SolanaWallet(this.provider);
        const accounts = await solanaWallet.requestAccounts();
        const connectionConfig = await solanaWallet.request<CustomChainConfig>({
          method: "solana_provider_config",
    ...
    console.log("public key:"+new PublicKey(accounts[0]));
    ...
    createNFT=async():Promise<string> => {
      try {
        
        let privateKey=await this.getPrivateKey();
        let res="";
    await fetch('http://localhost:4000/uploadNFT')
    ...
    //     let privateKey=await this.getPrivateKey();
    //     let res="";
    // await fetch('http://localhost:4000/createNFT/'+privateKey)
    // .then(response => response)
    // .then(json => json.text)
    // .catch(error => console.error(error));
    //   return res ;
